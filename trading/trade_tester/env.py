...
class TradingEnv(Env):
    ...
    def __init__(self, klines: pd.DataFrame, window: int = 100,
        ...
        Args:
            ...
            window (int, optional): Observation window of history distance. Defaults to 100.
            ...
            features (list): list of features accessible by environment. If empty - all features in klines except date.
        """
        # 0 - Sell, 1 - Buy, 2 - Pass
        ...
        if features:
            ...
            self.features = klines.drop(['date'], axis=1).columns
        self.observation_space = Box(
            shape=(window, len(self.features)),
            dtype=np.float32,
            low=0,
            high=1
        )
    ...
    def _get_observations(self, as_df=False) -> np.ndarray:
        """Return an observation"""
        obs = create_observation_window(
            df=self.tester.klines[self.features],
            n=self.tester.n_tick,
